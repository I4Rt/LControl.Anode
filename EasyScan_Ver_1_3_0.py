...
def get_data_cut(center, data_3d_inside):
    ...
    real_cut = [[item[0], item[1], item[2]] for item in data_3d_inside if
                real_center - 0.0012 < item[2] < real_center + 0.0012]  # changed
...
s = serial.Serial('COM5', 57600, parity=serial.PARITY_NONE, stopbits=serial.STOPBITS_ONE, bytesize=serial.EIGHTBITS,
                  timeout=0.1)
